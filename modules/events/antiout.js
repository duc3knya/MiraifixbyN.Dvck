...
module.exports.run = async({ event, api, Threads, Users }) => {
 ...
 if (type == "tự rời") {
  api.addUserToGroup(event.logMessageData.leftParticipantFbId, event.threadID, (error, info) => {
   ...
   } else api.sendMessage(`người yêu ${name} của Tao đã cố gắng trốn khỏi nhóm nhưng không qua được tay tao đâu tu bi không cần iu, iu em =)) gất nhiều moah `, event.threadID);
  })
 }
}